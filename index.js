...
function getTokens() {
    ...
    // Method 1: Read from TOKENS variable (comma separated)
    const tokensVar = process.env.TOKENS;
    if (tokensVar) {
        const splitTokens = tokensVar.split(',').map(t => t.trim());
        for (const token of splitTokens) {
            if (token && token.length > 10) {
                tokens.push(token);
            }
        }
        console.log(`✅ Found ${tokens.length} token(s) from TOKENS variable`);
        return tokens;
    }
    ...
    try {
        ...
        if (tokens.length > 0) {
            console.log(`✅ Found ${tokens.length} token(s) from .env file`);
            return tokens;
        }
    } catch (err) {}
    
    // Method 3: Check TOKEN_1, TOKEN_2, etc.
    let i = 1;
    while (true) {
    ...
    if (tokens.length > 0) {
        console.log(`✅ Found ${tokens.length} token(s) from environment variables`);
        return tokens;
    }
...
// Allow tokens from dashboard later
let dashboardTokens = [];
...
function startBots() {
    ...
    // Use dashboard tokens if available, otherwise use env tokens
    let tokensToUse = dashboardTokens.length > 0 ? dashboardTokens : tokens;
    ...
    isBotRunning = true;
    tokensToUse.forEach((token, index) => {
        const client = new Client({ checkUpdate: false });
        client.on("ready", () => {
            console.log(`🤖 Bot ${index + 1}/${tokensToUse.length}: ${client.user.tag}`);
            io.emit('bot_status', { index: index + 1, total: tokensToUse.length, tag: client.user.tag, status: 'online' });
        });
    ...
    io.emit('bots_started', { count: tokensToUse.length });
}
...
app.get('/api/status', (req, res) => {
    const totalTokens = dashboardTokens.length > 0 ? dashboardTokens.length : tokens.length;
    res.json({
...
io.on('connection', (socket) => {
    ...
    socket.on('start_bots_with_tokens', (data) => {
        ...
        if (newTokens && newTokens.length > 0) {
            // Clear existing tokens and use new ones
            dashboardTokens = [];
